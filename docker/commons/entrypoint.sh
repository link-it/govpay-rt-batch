...
##############################################################################
# Configurazione URL Base pagoPA RT API
##############################################################################

# Priorita' 1: URL custom (se specificato, ha precedenza)
if [ -n "${GOVPAY_RT_API_CUSTOMURL}" ]; then
    PAGOPA_RT_BASE_URL="${GOVPAY_RT_API_CUSTOMURL}"
    log_info "Utilizzo URL RT API personalizzato: ${PAGOPA_RT_BASE_URL}"
# Priorita' 2: Selezione URL basata su ambiente
elif [ -n "${GOVPAY_RT_API_ENV}" ]; then
    # Conversione a minuscolo per confronto case-insensitive
    RT_ENV=$(echo "${GOVPAY_RT_API_ENV}" | tr '[:upper:]' '[:lower:]')

    case "${RT_ENV}" in
        prod|produzione)
            PAGOPA_RT_BASE_URL="https://api.platform.pagopa.it/nodo/nodo-per-pa/v1"
            log_info "Utilizzo ambiente RT API PRODUZIONE"
            ;;
        collaudo|test|stage|uat)
            PAGOPA_RT_BASE_URL="https://api.uat.platform.pagopa.it/nodo/nodo-per-pa/v1"
            log_info "Utilizzo ambiente RT API UAT/TEST"
            ;;
        *)
            log_error "Valore GOVPAY_RT_API_ENV non valido: ${GOVPAY_RT_API_ENV}"
            log_error "Valori ammessi: collaudo, test, stage, uat, prod, produzione"
            exit 1
            ;;
    esac
    log_info "URL Base RT API: ${PAGOPA_RT_BASE_URL}"
fi

# Validazione configurazione RT API obbligatoria
if [ -z "${PAGOPA_RT_BASE_URL}" ]; then
    log_error "Configurazione URL Base RT API mancante"
    log_error "Impostare una di: GOVPAY_RT_API_CUSTOMURL o GOVPAY_RT_API_ENV"
    exit 1
fi
export PAGOPA_RT_BASE_URL

if [ -z "${GOVPAY_RT_API_SUBSCRIPTIONKEY}" ]; then
    log_error "Subscription Key RT API mancante"
    log_error "Richiesta: GOVPAY_RT_API_SUBSCRIPTIONKEY"
    exit 1
fi
export PAGOPA_RT_SUBSCRIPTION_KEY="${GOVPAY_RT_API_SUBSCRIPTIONKEY}"
...
# Integrazione GDE disabilitata di default a meno che non venga fornito GOVPAY_RT_GDE_URL
if [ -n "${GOVPAY_RT_GDE_URL}" ]; then
    GOVPAY_GDE_ENABLED=true
    GOVPAY_GDE_BASE_URL="${GOVPAY_RT_GDE_URL}"
...
case "${GOVPAY_RT_BATCH_USA_CRON,,}" in
    ...
    *)
        ...
        SCHEDULER_RTACQUISITIONJOB_FIXEDDELAYSTRING=$((INTERVALLO_MINUTI * 60 * 1000))

        export SERVER_PORT SCHEDULER_RTACQUISITIONJOB_FIXEDDELAYSTRING
        log_info "Porta Actuator: ${SERVER_PORT}, Intervallo scheduler: ${INTERVALLO_MINUTI} minuti (${SCHEDULER_RTACQUISITIONJOB_FIXEDDELAYSTRING}ms)"
        ;;
esac
...
log_info "RT API: ${PAGOPA_RT_BASE_URL}"
