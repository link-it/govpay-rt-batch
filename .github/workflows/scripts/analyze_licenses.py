...
# Database esteso compatibilita' licenze
...
def analyze_licenses(exceptions_file=None):
    ...
    for dep in dependencies:
        ...
        # Gestione dual/multiple licensing - scegli la licenza piu' compatibile
        best_license = None
        ...
        for lic in dep['licenses']:
            ...
            # Ottieni compatibilita' per questa licenza
            ...
            # Calcola score di compatibilita' (priorita': GPLv3 + Enterprise > solo Enterprise > solo GPLv3 > nessuno)
            score = 0
        ...
        if best_license is None or best_license.get('name') == 'Unknown':
            print(f"  No license found for {group_id}:{artifact_id}, attempting to fetch from Maven Central...")
            ...
            if fetched_licenses:
                ...
                for lic in fetched_licenses:
                    ...
                    # Calcola score di compatibilita'
                    score = 0
                    if compat['gplv3_compatible'] == True:
                        score += 2
                    if compat['enterprise_safe'] == True:
        ...
        # Se c'e' dual licensing, mostra tutte le licenze nel nome
        ...
        # Check se c'e' un'eccezione per questo artifact
        ...
        # Identifica problemi di compatibilita'
        # (exception gia' definito sopra)
        ...
        if gplv3_compat is None and enterprise_safe is None:
            if exception:
                print(f"  Eccezione gestita: {artifact_key} - {license_name} (licenza sconosciuta/non classificata) - Motivo: {exception['reason']}")
            else:
            ...
            if gplv3_compat is False or gplv3_compat is None:
                if exception:
                    ...
                    print(f"  Eccezione gestita: {artifact_key} - {license_name} ({reason_text}) - Motivo: {exception['reason']}")
                else:
    ...
    # Combina tutti i problemi per il report (mantenendo retrocompatibilita')
    ...
    print(f"  - Problemi di compatibilita' GPLv3 NON gestiti: {len(gplv3_issues)}")
    print(f"  - Problemi di compatibilita' Enterprise NON gestiti: {len(enterprise_issues)}")
    ...
    if total_compatibility_issues == 0:
        ...
        if len(gplv3_issues) > 0:
            error_messages.append(f"{len(gplv3_issues)} problemi di compatibilita' GPLv3")
        if len(enterprise_issues) > 0:
            error_messages.append(f"{len(enterprise_issues)} problemi di compatibilita' Enterprise (ShareAlike/Copyleft)")
...
"""
    ...
    html += """
        ...
        <p style="font-size: 14px; color: #666; margin-bottom: 10px;">
            ...
            Artifacts with yellow background have managed exceptions.
        </p>
        <table>
    ...
    for group_id in sorted(artifacts_by_group.keys()):
        ...
        for artifact in artifacts:
            ...
            html += f"""<tr class="{row_class}">
                ...
                <td class="{gplv3_class}">{"OK" if artifact['gplv3_compatible'] else ("NO" if artifact['gplv3_compatible'] is False else "?")}</td>
                <td class="{enterprise_class}">{"OK" if artifact['enterprise_safe'] else ("NO" if artifact['enterprise_safe'] is False else "?")}</td>
...
def generate_markdown_report(data, output_dir):
    ...
    if gplv3_issue_only > 0:
        md += f"| GPLv3 issue only | No | Yes | {gplv3_issue_only} |\n"
    if enterprise_issue_only > 0:
        md += f"| Enterprise issue only | Yes | No | {enterprise_issue_only} |\n"
    if both_issues > 0:
